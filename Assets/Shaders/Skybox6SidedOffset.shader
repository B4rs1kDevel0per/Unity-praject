Shader "Skybox/6 Sided with Offset" {
Properties {
    _Tint ("Tint Color", Color) = (.5, .5, .5, .5)
    [Gamma] _Exposure ("Exposure", Range(0, 8)) = 1.0
    _Rotation ("Rotation", Range(0, 360)) = 0
    _VOffset ("Vertical Offset", Range(-2.0, 2.0)) = 0.0
...
SubShader {
    ...
    v2f vert (appdata_t v) {
        v2f o;
        float3 rotatedVertex = RotateAroundYInDegrees(v.vertex.xyz, _Rotation);
        // Смещаем координату Y для боковых и верхних/нижних текстур
        rotatedVertex.y -= _VOffset * length(rotatedVertex.xz);
        o.vertex = UnityObjectToClipPos(float4(rotatedVertex, v.vertex.w));
    ...
    half4 skybox_frag (v2f i, sampler2D smp) {
        ...
        half3 c = DecodeHDR (tex, half4(1,1,1,1));
